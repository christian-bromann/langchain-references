...
export function symbolToMarkdown(
  ...
  if (symbol.params && symbol.params.length > 0) {
    ...
    for (const param of symbol.params) {
      ...
      lines.push(
        `| \`${param.name}\`${required} | \`${param.type}\` | ${desc.replace(/\n/g, " ")} |`,
      );
    }
    lines.push("");
  }
...
import { ChevronRight, Check, ExternalLink, Copy, FileText, Link as LinkIcon } from "lucide-react";
...
function MenuItem({ icon, label, description, href, external, checked, onClick }: MenuItemProps) {
  const content = (
    <>
      <div className="border border-border rounded-lg p-1.5">{icon}</div>
      ...
      <Check
        className={cn("w-3.5 h-3.5 shrink-0 text-primary", checked ? "opacity-100" : "opacity-0")}
      />
    </>
  );
    ...
    for (const pkg of manifest.packages) {
      ...
      if (symbolsResult?.symbols) {
        ...
        if (classes.length > 0) {
          lines.push("### Classes");
          lines.push("");
          for (const cls of classes.slice(0, 100)) {
            // Limit for size
...
describe("Markdown Response", () => {
  it("should return markdown content when format=md", async () => {
    const request = new Request("http://localhost:3000/python/langchain-core/ChatOpenAI?format=md");
