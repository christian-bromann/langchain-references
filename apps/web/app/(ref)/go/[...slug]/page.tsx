 ...
 * Go Symbol Page
 *
 * Dynamic route handler for Go package and symbol pages.
...
/**
 * Generate static params for all Go packages and symbols.
 */
export async function generateStaticParams(): Promise<{ slug: string[] }[]> {
  ...
  const projectParams = await Promise.all(
    projects.map((project) => getStaticParamsForLanguage("go", project.id)),
  );
...
export default async function GoSymbolPage({ params, searchParams }: Props) {
  ...
  const parsed = parseSlugWithLanguage(slug, "go");
...
export async function generateMetadata({ params }: Props) {
  ...
  const description = parsed.fullPath
    ? `Go API reference for ${parsed.fullPath} in ${parsed.packageName}. Part of the LangChain ecosystem.`
    : `Go API reference for ${parsed.packageName}. Part of the LangChain ecosystem.`;

  const ogImagePath = `/og/go/${slug.join("/")}`;
