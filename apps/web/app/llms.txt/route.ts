...
export async function GET(): Promise<Response> {
    ...
    if (pythonManifest) {
      ...
      if (pythonPackages.length > 0) {
        ...
        for (const pkg of pythonPackages) {
          ...
          lines.push(
            `- [${pkg.publishedName}](${pkgUrl}): ${pkg.stats?.total || 0} symbols`
          );
        }
        lines.push("");
      }
  ...
  lines.push("");
  lines.push(
    "You can access any page as markdown by appending `?format=md` to the URL."
  );
  lines.push("");
  lines.push("**Examples:**");
  lines.push("");
  lines.push(
    `- \`${baseUrl}/python/langchain-core/RunnableSequence?format=md\``
  );
  lines.push(
    `- \`${baseUrl}/javascript/langchain-core/ChatPromptTemplate?format=md\``
  );
  lines.push("");
  lines.push(
    "The API also serves markdown automatically when detecting LLM user agents"
  );
  lines.push(
    "(GPTBot, Claude-Web, etc.) or when using curl/wget without Accept headers."
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `This documentation is available as an MCP (Model Context Protocol) server:`
  );
  lines.push("");
  lines.push(`\`${mcpServerUrl}\``);
  lines.push("");
  lines.push(
    "Use this URL to connect from Cursor, VS Code, or other MCP-compatible tools."
  );
  lines.push("");
